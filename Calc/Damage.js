...
class DamageCalculator {
  ...
  calculateDamage() {
    ...
    for (let i = 0; i < this.attackCount; i++) {
      ...
      if (hasTrueDamage && hasPierceDamage) {
        // True + Pierce: bỏ qua %Res và FixedRes, nhân với ElementRes >= 1
        damageTaken = attackValue * Math.max(resistance, 1);
        formula = `${attackValue} * ${Math.max(resistance, 1)}`;
      } else if (hasTrueDamage) {
        // True: bỏ qua %Res, trừ FixedRes, nhân với ElementRes >= 1
        ...
        // Pierce: áp dụng %Res, bỏ qua FixedRes
      ...
      results.push({
        damage: damageTaken,
        message: `Công thức: ${formula} <br> ➜ <strong>Sát thương nhận vào: ${damageTaken}</strong>`
      });
    }
...
class DamageUIManager {
  constructor() {
    ...
    // append vào cùng form-actions với addAttackBtn
    const formActions = this.addAttackBtn.parentNode;
    formActions.appendChild(this.clearBtn);
  }
    ...
    for (let i = 0; i < count; i++) {
      ...
      group.innerHTML = `
        ...
        <div class="form-group">
          ...
          <button class="btn small add" onclick="addDefense(${i})"><i class="fas fa-plus"></i> Thêm % / Cố định</button>
        ...
        <div class="form-group">
          <label><input type="checkbox" id="antiTrue${i}"> Anti True Damage</label>
        </div>
        <div class="form-group">
          <label><input type="checkbox" id="antiPierce${i}"> Anti Pierce Damage</label>
        </div>
...
class DamageController {
  ...
  saveData() {
    const data = { attackCount: this.attackCount, attacks: [] };
    for (let i = 0; i < this.attackCount; i++) {
      const effect = [];
...
function addDefense(index) {
  ...
  group.innerHTML = `
    ...
    <button class="btn small clear" onclick="removeDefense(${index}, ${count})"><i class="fas fa-trash"></i> Xóa</button>
  `;
